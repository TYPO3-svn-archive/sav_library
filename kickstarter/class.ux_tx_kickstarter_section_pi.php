...
***************************************************************/
/**
 * @author	Kasper Sk?j <kasperYYYY@typo3.com>
 */
...
//--------------------------
            ...
            $innerMainContent = $this->add_2_tx_kickstarter_section_pi($cN);					   					
 					} else {
// Begin - Original code
...
// Generate the code for the extension
  function add_2_tx_kickstarter_section_pi($cN) {
//debug($this->wizard->wizArray,'','','',10);
//debug($this->wizard);
...
//debug($showFolders,'$showFolders');
                ...
                if (isset($showFolders)) {  
                  ...
                  foreach ($showFolders as $keyFolder => $folder) {
                    ...
                    // Get the folder config parameter
            			  $xmlArray['views'][$k][$folderName]['config'] = $this->getConfig($opt_showFolders[$keyFolder]['parameters']);
                    ...
                    $xmlArray['views'][$k][$folderName]['title'] = $title[$k];
                   
                    // Generate the addPrintIcon information
                    if ($view['showAddPrintIcon']) { 
                      $xmlArray['views'][$k][$folderName]['addPrintIcon'] = $view['showAddPrintIcon'];
                      if ($view['relview']) {
                        $xmlArray['views'][$k][$folderName]['relViewPrintIcon'] = $view['relview'];
                      }
                    }
                    ...
                    foreach($folder as $key => $value) {
                      ...
                      if ($field['conf_showFieldDisp'][$k]) { 
                        ...
                        if ($field['type']=='rel' && $field['conf_rel_type'] == 'group') {           
                          $relTable[$field['conf_rel_table']] = $name;                         
                        } 
                        
                        $config = $this->getConfig($field['conf_showField'][$k]) + $config;
                        $configTemp[$name] = array('folderName' => $folderName, 'config' => $config);                                                   
                      }
                    }
                  } 
                  ...
                  if(is_array($configTemp)) {
                    ...
                    foreach($configTemp as $keyTemp => $valTemp) {  
                      ...
                      if (array_key_exists($valTemp['config']['table'], $relTable)) {
                        ...
                        if (is_array($xmlArray['views'][$k][$valTemp['folderName']]['fields'][$relTable[$valTemp['config']['table']]])) {                       
                          $xmlArray['views'][$k][$valTemp['folderName']]['fields'][$relTable[$valTemp['config']['table']]]['config'][0]['fields'][$keyTemp]['config'] = $valTemp['config'];
                        } else {
                          // Check if the folder is the than the the folder of the subform
                          if ($valTemp['folderName']==$configTemp[$relTable[$valTemp['config']['table']]]['folderName']) {  
                            ...
                            // just set the config
                            $xmlArray['views'][$k][$valTemp['folderName']]['fields'][$keyTemp]['config'] = $valTemp['config'];                          
                          }                                               
                        }                
                      } else {
                        $xmlArray['views'][$k][$valTemp['folderName']]['fields'][$keyTemp]['config'] = (array) $xmlArray['views'][$k][$valTemp['folderName']]['fields'][$keyTemp]['config'] + $valTemp['config'];
                        if (array_search($keyTemp, $relTable) && is_array($temp)) {
                          foreach($temp as $key=>$value) {
                            if ($value['table'] == array_search($keyTemp, $relTable)) {
                              $xmlArray['views'][$k][$valTemp['folderName']]['fields'][$relTable[$value['table']]]['config'][0]['fields'][$key]['config'] = $value;
                              unset($temp[$key]);
                            }
                          }
                        }
            ...
            // check if a php expression is used            
            if (preg_match_all('/\{PHP:(.*?)\}/', $configString, $matches)) {
           
              foreach ($matches[1] as $keyMatch => $match) {
                $configString = str_replace($matches[0][$keyMatch], '\'.'.str_replace('\\\'', '\'', $match).'.\'', $configString);
              }
            }                   
